...
const repService = import('../../../services/bidding-reports-service')
...
export class Summary {
  ...
  async __getBiddingSummary (json) {
    const serv = (await repService).default
    return new serv().summary({
  ...
  changeDate () {
    ...
    if (from.value.length && to.value.length) {
      ...
      this.create({
          ...
          changeBiddingSummaryDOM = new CustomEvent('changeBiddingSummaryDOM', { detail: json })
  ...
  loadChangeDate (e) {
    ...
    return import('../../../components/report-change-date-modal').then(loader => { 
      const __target = document.querySelector('#general-modal > .content > .body')
      __target.innerHTML = loader.template
      __target.querySelector('#modal-dialog-send-button').addEventListener('click', this.changeDate.bind(__proto))
...
const changeGraphDisplay = (json) => {
  ...
  if (json.from) {
    ...
    import('../../../config/api').then(net => {
      targ.href = `${net.default.url}/bidding/reports/bidding_summary.php?from=${json.from}&to=${json.to}`
    }) 
  }
  // total
  if (json.total) {
            ...
            <div class="card-body">
              <div class="media">
                ...
                <div class="media-body">
                    <p class="mt-0"><b>${json.department[val].alias || 'Others'}</b> <br/>
                      <span class="text-muted">${json.department[val].name || 'N/A'}</span><br/>
