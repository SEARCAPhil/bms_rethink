...
const BiddingServ = import('../../../services/bidding-proposal-attachment-service')
...
export default class {
  ...
  async removeAttachments (e) {
    ...
    // remove from DB
    return new serve().remove(__payload).then(res => { console.log(res.data)
      return res.data == 1 ?  this.success(__payload.id) : this.error()
    }).catch(err => this.error(err) | console.log(error))
  }
