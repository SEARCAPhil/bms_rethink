...
export class FeedList {
	...
	get (id) { 
		...
		return new Promise((resolve, reject) => {
			this.FeedServ.getPerBidding(data).then((json) => {
...
document.querySelectorAll('.feedback-bidding-list-section').forEach((el, index) => {
	const Feed = new FeedList()
	const id = el.id
	Feed.get(id).then((json) => {
		if (json.length > 0) {
			el.innerHTML = '<p style="border-bottom:1px solid rgba(200,200,200,0.3);padding-bottom:10px;margin-top: 60px;"><b>Customer Reviews</b> <i class="material-icons md-18 float-right text-muted">expand_more</i></p>'
		}
			...
			art.innerHTML+=`
						<article class="row col-12">
						    <section class="col-12 col-lg-7">
						    	...
						    	<div class="media">
								  ...
								  <div class="media-body">
								    <p class="mt-0"><b>${val.author[0].profile_name}</b><br>
										${val.author[0].department}<br/>
										<a href="${window.bms.config.network}/bidding/reports/bidding_feedback.php?id=${val.id}" target="_blank" class="btn btn-default btn-sm" style="border:1px solid #009688;">
								...
								<p class="text-muted">${val.feedback}<br/><span class="badge badge-dark">${val.company_name}</span> <span class="badge badge-dark">${val.product_name}</span></p>
						    </section>
